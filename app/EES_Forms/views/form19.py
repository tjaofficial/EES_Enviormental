...
from ..utils import fix_data, get_initial_data, weatherDict, method9_reading_data_build, form19_ovens_data_build
...
def form19(request, facility, fsID, selector):
    ...
    # -----SET RESPONSES TO DECIDING VARIABLES------------
        ...
        # if selector = 'formL' and orgFormL.exists():
        #     formName = 'H-L'
        ...
        if search:
            ...
            if existing:
                ...
                unparsedData = get_initial_data(form19_model, database_form)
                initial_data = {
            ...
            data = form19_form(initial=initial_data, form_settings=form_variables['freq'])
    # -----IF REQUEST.POST------------
        if request.method == "POST":
            print(request.POST)
    # -----CREATE COPYPOST FOR ANY ADDITIONAL INPUTS------------
            dataCopy = request.POST.copy()
            dataCopy['ovens_data'] = form19_ovens_data_build(request.POST)
            ...
            dataCopy['reading_data']['comb_formL'] = False
    ...
    # -----SET FORM VARIABLE IN RESPONSE TO DECIDING VARIABLES------------
            if existing:
                ...
                form = form19_form(dataCopy, instance=database_form, form_settings=form_settings) 
                ...
                form = form19_form(dataCopy, form_settings=form_settings)
    ...
    return render(request, "shared/forms/weekly/form19.html", {
        'fsID': fsID, 
